...
import { 
  ...
  PartyPopper, CheckCircle, Music, Plus, User, Disc, Radio
} from 'lucide-react';
...
export interface PlayerContextType extends PlayerState {
  ...
  playPlaylist: (tracks: JamendoTrack[], startIndex?: number) => void;
  togglePlay: () => void;
  nextTrack: () => void;
  prevTrack: () => void;
...
interface ExtendedPlayerContextType extends PlayerContextType {
  favorites: JamendoTrack[];
  toggleFavorite: (track: JamendoTrack) => void;
  isFavorite: (trackId: string) => boolean;
  setSleepTimer: (minutes: number | null) => void;
  sleepTimerTime: number | null;
}
...
const setCache = (key: string, data: any) => cache.set(key, { timestamp: Date.now(), data });
...
// --- Jamendo Service ---
const fetchJamendo = async (endpoint: string, params: Record<string, string>): Promise<JamendoTrack[]> => {
    ...
    try {
        ...
        const tracks = data.results.map((raw: any) => ({
            id: raw.id,
            name: raw.name,
...
const searchJamendo = (query: string) => fetchJamendo('/tracks/', { search: query });
const getTracksByTag = (tag: string, limit = 20) => fetchJamendo('/tracks/', { limit: limit.toString(), tags: tag, boost: 'popularity_month' });
const getTracksByArtist = (artistId: string, limit = 20) => fetchJamendo('/tracks/', { limit: limit.toString(), artist_id: artistId, order: 'popularity_total' });

// --- Saavn Service ---
const SAAVN_BASE_URLS = ['https://saavn.dev.api.avishay.co.il', 'https://saavn.me', 'https://jiosaavn-api-privatecvc2.vercel.app'];

const mapSaavnToJamendo = (track: any): JamendoTrack | null => {
  // Normalize Image - FORCE HIGH RES
  ...
  // Replace 150x150 with 500x500 for better UI quality
  ...
  const artistName = decodeHtml(track.primaryArtists || track.artist || 'Unknown Artist');
...
const searchSaavn = async (query: string): Promise<JamendoTrack[]> => {
  ...
  for (const baseUrl of SAAVN_BASE_URLS) {
      try {
          ...
          if (results.length > 0) {
              const tracks = results.map(mapSaavnToJamendo).filter((t): t is JamendoTrack => t !== null);
              if (tracks.length > 0) {
...
const analyzeTasteProfile = (history: JamendoTrack[]) => {
...
const getDailyMix = async (profile: any) => {
    const promises = [];
    ...
    if (profile.preferredSource !== 'saavn') {
        const tag = profile.topTags.find((t: string) => t !== 'desi') || 'pop';
...
const getTimeBasedRecommendations = async (profile: any) => {
...
const getMoodTracks = async (mood: string) => {
    const map: any = {
...
// --- Auth Context ---
interface AuthContextType { currentUser: FirebaseUser | null; loading: boolean; logout: () => Promise<void>; }
...
// --- Player Context ---
const PlayerContext = createContext<ExtendedPlayerContextType | undefined>(undefined);
...
const PlayerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [sleepTimerTime, setSleepTimerTime] = useState<number | null>(null);
  ...
  const playTrack = (t: JamendoTrack) => setState(p => ({ ...p, currentTrack: t, isPlaying: true, queue: [t] }));
  const playPlaylist = (t: JamendoTrack[], i = 0) => setState(p => ({ ...p, queue: t, currentTrack: t[i], isPlaying: true }));
  const togglePlay = () => setState(p => p.currentTrack ? { ...p, isPlaying: !p.isPlaying } : p);
  const setVolume = (v: number) => setState(p => ({ ...p, volume: v }));
  const seek = (t: number) => { if (audioRef.current) { audioRef.current.currentTime = t; setState(p => ({ ...p, currentTime: t })); } };
  ...
  const addToQueue = (t: JamendoTrack) => setState(p => ({ ...p, queue: [...p.queue, t] }));
  const removeFromQueue = (i: number) => setState(p => { const q = [...p.queue]; q.splice(i, 1); return { ...p, queue: q }; });
  const reorderQueue = (f: number, t: number) => setState(p => { const q = [...p.queue]; const [rm] = q.splice(f, 1); q.splice(t, 0, rm); return { ...p, queue: q }; });
  ...
  const toggleFavorite = (t: JamendoTrack) => setFavorites(prev => {
...
const Sidebar: React.FC<{ currentView: string; onChangeView: (v: string) => void; onCreatePlaylist: () => void; }> = ({ currentView, onChangeView, onCreatePlaylist }) => {
...
const MobileNav: React.FC<{ currentView: string; onChangeView: (v: string) => void }> = ({ currentView, onChangeView }) => (
...
const SearchBar: React.FC<{ onSearch: (q: string) => void; initialQuery?: string }> = ({ onSearch, initialQuery='' }) => {
    ...
    useEffect(() => { const h = (e: any) => { if(rRef.current && !rRef.current.contains(e.target)) setF(false); }; document.addEventListener('mousedown', h); return () => document.removeEventListener('mousedown', h); }, []);
    
    const addR = (t: string) => { const n = [t, ...rec.filter(x=>x!==t)].slice(0,5); setRec(n); localStorage.setItem('vex_rec_search', JSON.stringify(n)); };
    const click = (t: string) => { setQ(t); setF(false); onSearch(t); addR(t); };
    return (
...
const TrackList: React.FC<{ tracks: JamendoTrack[]; onPlay: (t: JamendoTrack, l: JamendoTrack[]) => void; onArtistClick?: (id: string, name: string) => void; title?: string; showCover?: boolean }> = ({ tracks, onPlay, onArtistClick, title, showCover=true }) => {
    const { currentTrack, isPlaying, toggleFavorite, isFavorite } = usePlayer();
    const format = (s: number) => `${Math.floor(s/60)}:${Math.floor(s%60).toString().padStart(2,'0')}`;
    return (
...
const Section: React.FC<{ title: string; subtitle?: string; tracks: JamendoTrack[]; onPlay: (t: JamendoTrack) => void; onArtistClick: (id: string, name: string) => void }> = ({ title, subtitle, tracks, onPlay, onArtistClick }) => {
...
const Player: React.FC = () => {
  ...
  useEffect(() => { const h = (e: MouseEvent) => { if (mRef.current && !mRef.current.contains(e.target as Node)) setMore(false); }; if(more) document.addEventListener('mousedown', h); return () => document.removeEventListener('mousedown', h); }, [more]);
...
const ContentArea: React.FC = () => {
  ...
  const [daily, setDaily] = useState<JamendoTrack[]>([]);
  const [timeRec, setTimeRec] = useState<JamendoTrack[]>([]);
  const [explore, setExplore] = useState<{title:string,tracks:JamendoTrack[]}[]>([]);
  const [moodList, setMoodList] = useState<JamendoTrack[]>([]);
  const [topRes, setTopRes] = useState<JamendoTrack|null>(null);
  ...
  const [selPl, setSelPl] = useState<Playlist|null>(null);
  ...
  const onArtistClick = async (id: string, name: string) => {
  ...
  const onPlay = (t: JamendoTrack, list: JamendoTrack[]) => {
